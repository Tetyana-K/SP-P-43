...
Process currentProcess = Process.GetCurrentProcess(); // отримуємо інформацію про поточний процес,
                                                      // повертається об'єкт типу Process, який представляє поточний процес
...
while (true)
{
    var processes = Process.GetProcesses().OrderBy(p => p.ProcessName); // отримуємо список всіх процесів,
                                                                        // впорядковуючи їх за назвою
    foreach (var process in processes)
    {
        ...
        catch (Exception ex) // може виникнути, якщо процес завершився між отриманням списку процесів і спробою доступу до його властивостей, або якщо у нас недостатньо прав для доступу до інформації про процес
        {
...
try
{
    Process proc = Process.GetProcessById(pid); // отримуємо процес за його PID
    ...
    Console.WriteLine($"Threads number:     {proc.Threads.Count}"); // кількість потоків, які належать процесу

    int sameNamed = Process.GetProcessesByName(proc.ProcessName) // отримуємо всі процеси з такою ж назвою, як у поточного процесу
        .Length; //  кількість процесів з такою ж назвою, як у поточного процесу
...
void PrintProcessInfo(Process process)
{
    ...
    Console.WriteLine($"Process: {process.ProcessName}"); // назва процесу
    Console.WriteLine($"ID: {process.Id}"); // унікальний ідентифікатор процесу (PID)
    ...
    Console.WriteLine($"Main windows title: {process.MainWindowTitle}"); // заголовок головного вікна процесу (якщо є), якщо процес не має вікна або не має заголовка, то буде пустий рядок
    Console.WriteLine($"Priority: {process.PriorityClass}"); // пріоритет процесу (можливі значення від 0 до 31, де 0 - найнижчий пріоритет, а 31 - найвищий)
    Console.WriteLine($"Start time: {process.StartTime}");

    // Включає користувацький час (User) + системний час (Kernel)
    Console.WriteLine($"Total CPU time: {process.TotalProcessorTime}"); // загальний час процесора, витрачений процесом

    // Час, витрачений  на користувацький код
    Console.WriteLine($"User Processor Time: {process.UserProcessorTime}"); // час процесора, витрачений на виконання користувацького коду
    Console.WriteLine($"PrivateMemorySize (KB): {process.PrivateMemorySize64 / 1024.0} KB"); // обсяг приватної пам'яті процесу
